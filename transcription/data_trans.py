...
class PianoSampleDataset(Dataset):
    def __init__(self, path, groups=None, sample_length=16000*5, seed=1, 
        ...
        self.path = path
        self.path_p = Path('/workspace/AutoregressivePianoTranscription/data/maestro_infer/w0.9')
        self.path_r = Path('/workspace/AutoregressivePianoTranscription/data/maestro_infer/w1.3')
    ...
    def __getitem__(self, index):
            ...
            for el in self.frame_features:
                ...
                if step_begin > self.delay - 1:
                    ...
                    result[el] = F.pad(
                       ...
                       (0,0,self.delay,0))
            result['pred_p'] = th_load_from_memmap(
                self.path_p / Path(audio_path).name.replace('.flac', '_pred.npy'), np.uint8, 
                step_begin*88*np.dtype(np.uint8).itemsize, (n_steps, 88), np.int64)
            result['pred_p_vel'] = th_load_from_memmap(
                self.path_p / Path(audio_path).name.replace('.flac', '_vel.npy'), np.uint8, 
                step_begin*88*np.dtype(np.uint8).itemsize, (n_steps, 88), np.int64)
            result['pred_r'] = th_load_from_memmap(
                self.path_r / Path(audio_path).name.replace('.flac', '_pred.npy'), np.uint8, 
                step_begin*88*np.dtype(np.uint8).itemsize, (n_steps, 88), np.int64)
            result['pred_r_vel'] = th_load_from_memmap(
                self.path_r / Path(audio_path).name.replace('.flac', '_vel.npy'), np.uint8, 
                step_begin*88*np.dtype(np.uint8).itemsize, (n_steps, 88), np.int64)
        ...
        result['pred_p_vel'] = th.clamp(result['pred_p_vel'], 0, 127)
        result['pred_r_vel'] = th.clamp(result['pred_r_vel'], 0, 127)
