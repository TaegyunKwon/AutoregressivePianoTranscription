...
class PianoSampleDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.sample_length is not None:  # fixed length segmentation
            ...
            result['bw'] = th_load_from_memmap(
                ...
                step_begin*n_feature*np.dtype(save_dtype).itemsize, (n_steps, n_feature), np.int64)
            result['bw_v'] = th_load_from_memmap(
                audio_path.replace('.flac', '_bw_v.npy'), np.uint8,
                step_begin*n_feature*np.dtype(save_dtype).itemsize, (n_steps, n_feature), np.int64)
    ...
    def make_bw_condition(self, label, velocity):
    ...
    def generate_bw_condition(self, audio_path):
        meta_path = audio_path.replace('.flac', '_meta.pt')
        n_steps = th.load(meta_path)['n_steps']
        cast_type = np.int64
        save_dtype = np.uint8
        label = th_load_from_memmap(
                        audio_path.replace('.flac', '_label.npy'), save_dtype, 0,
                        (n_steps, 88), cast_type)
        velocity = th_load_from_memmap(
                        audio_path.replace('.flac', '_velocity.npy'), save_dtype, 0,
                        (n_steps, 88), cast_type)
        bw_c, bw_v = self.make_bw_condition(label, velocity)
        save_to_memmap(bw_c.numpy(), (n_steps, 88), 'uint8',
                       audio_path.replace('.flac', '_bw.npy'))
        save_to_memmap(bw_v.numpy(), (n_steps, 88), 'uint8',
                       audio_path.replace('.flac', '_bw_v.npy'))
    ...
    def load(self, audio_path, tsv_path):
        ...
        bw_c, bw_v = self.make_bw_condition(label, velocity)
        save_to_memmap(bw_c.numpy, (n_steps, n_keys), 'uint8',
                       audio_path.replace('.flac', '_bw.npy'))
        save_to_memmap(bw_v.numpy, (n_steps, n_keys), 'uint8',
                       audio_path.replace('.flac', '_bw_v.npy'))
