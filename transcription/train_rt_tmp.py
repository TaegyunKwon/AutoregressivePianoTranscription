...
from .model_rt_tmp import ARModel
...
class MovingBuffer:
    """Moving buffer for tracking gradient norms and computing quantiles for adaptive gradient clipping."""
...
def train_step(model, batch, loss_fn, optimizer, scheduler, device, config, gradNormHist=None, gradClippingQuantile=0.8, mode='gt'):
    ...
    if mode == 'gt':
        ...
        frame_out, vel_out, adaptive_label = model(audio, 
    ...
    # Adaptive gradient clipping based on quantile of gradient norm history
    if gradNormHist is not None:
        curClipValue = gradNormHist.getQuantile(gradClippingQuantile)
        totalNorm = clip_grad_norm_(model.parameters(), curClipValue)
        gradNormHist.step(totalNorm.item())
    else:
        # Fallback to fixed clipping if gradNormHist is not provided
        totalNorm = clip_grad_norm_(model.parameters(), 3.0)

    optimizer.step()
    # OneCycleLR handles warmup internally, so we call step() every iteration
    scheduler.step()
...
def valid_step(model, batch, loss_fn, device, config):
    ...
    # have to make it.
    frame_out, vel_out = model(audio, last_states=None, random_condition=False, mode='inference')
...
def train(rank, world_size, config, ddp=True):
    ...
    if rank == 0:
        if config.resume_dir:
            run = wandb.init(id=config.id, dir=config.logdir)
        else:   
            run = wandb.init(config=config, id=config.id, name=config.name, dir=config.logdir)
        summary(model)
    if ddp:
        model = th.nn.SyncBatchNorm.convert_sync_batchnorm(model)
        model = DDP(model, device_ids=[rank])
    # Use weight_decay from config if available, otherwise use default
    weight_decay = getattr(config, 'weight_decay', 1e-4)
    optimizer = AdaBelief(model.parameters(), lr=config.lr, 
                          eps=1e-16, betas=(0.9,0.999), weight_decouple=True, 
                          rectify=False, print_change_log=False, weight_decay=weight_decay)
    if config.resume_dir:
        ckp = th.load(model_saver.logdir / model_saver.last_ckp, map_location={'cuda:0':f'cuda:{rank}'}, weights_only=False)
        if ddp:
    ...
    if not config.eval:
        ...
        # Use OneCycleLR with warmup for more stable training (similar to train.py)
        scheduler = OneCycleLR(optimizer, max_lr=config.lr, total_steps=config.iteration, 
        ...
        # Initialize gradient norm history for adaptive gradient clipping
        gradNormHist = MovingBuffer(initValue=40, maxLen=10000)
        gradClippingQuantile = getattr(config, 'gradClippingQuantile', 0.8)
        ...
        for epoch in range(10000):
            ...
            for batch in data_loader_train:
                ...
                if step % config.valid_interval == 0 or step in [1, 5000]:
                    ...
                    if ddp:
                        dist.barrier()
                    # free gpu ram to prevent out of memory
                    del batch_metric, batch
                    th.cuda.empty_cache()
            if step > config.iteration:
                break
    ...
    ckp = th.load(model_saver.logdir / model_saver.best_ckp, map_location=map_location, weights_only=False)
    if ddp:
        model.module.load_state_dict(ckp['model_state_dict'])
    else:
        model.load_state_dict(ckp['model_state_dict'])
