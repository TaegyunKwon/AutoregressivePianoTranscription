...
class ARModel(nn.Module):
    def __init__(self, config, perceptual_w=False):
        ...
        if self.model == 'ConvTransEnc':
            self.acoustic = ConvTransEnc(config.n_mels, config.cnn_unit, config.hidden_per_pitch, use_film=config.film,
                                         cnn_widths=config.cnn_widths)
        elif self.model == 'ConvTransEnc2':
            self.acoustic = ConvTransEnc2(config.n_mels, config.cnn_unit, config.hidden_per_pitch, use_film=config.film,
                                         cnn_widths=config.cnn_widths)
        else:
    ...
    def forward(self, audio, last_states=None, last_onset_time=None, last_onset_vel=None, 
        ...
        if mode == 'gt':
            ...
            if adaptive_label is not None:
                ...
                # frame_out predicts label[:, 1:].
                # So we return adaptive_label[:, 1:].
                return frame, vel, adaptive_label[:, 1:]
...
class ConvTransEnc(nn.Module):
    def __init__(self, n_mels, cnn_unit, hidden_per_pitch, use_film,
                 cnn_widths = [3,3,3,3,3,3], multifc=True):
        super().__init__()
        ...
        self.cnn = nn.Sequential(
            ...
            nn.MaxPool2d((2, 1)),
            nn.Dropout(0.25),
            FilmBlock(cnn_unit, cnn_unit*2, n_mels//2, use_film=use_film, width_l1=cnn_widths[2], width_l2=cnn_widths[3]),
            nn.MaxPool2d((2, 1)),
            FilmBlock(cnn_unit*2, cnn_unit*4, n_mels//4, use_film=use_film, width_l1=cnn_widths[4], width_l2=cnn_widths[5]),
            nn.Dropout(0.25),
        )
        self.transformer = TransformerEncoder(cnn_unit*4, pos_embed_init_gamma=10.0, num_heads=8, fourier_size=16, hidden_factor=4, hidden_factor_attn=1, dropout=0.0, num_layers=4)
    ...
    def forward(self, mel):
        ...
        x = x.transpose(2,3)  # B 1 F L
        x = self.cnn(x)  # B C F L
        x = self.transformer(x.permute(0, 1, 3, 2), outputIndices=th.tensor(list(range(21, 108+1)), device=x.device))  # B L P D
        return x.permute(0, 1, 3, 2) # B L D P

class ConvTransEnc2(nn.Module):
    def __init__(self, n_mels, cnn_unit, hidden_per_pitch, use_film,
                 cnn_widths = [3,3,3,3,3,3], multifc=True):
        super().__init__()
        ...
        self.cnn = nn.Sequential(
                ...
                nn.Conv2d(1, cnn_unit, 3, padding = 1, stride = (2,1)),
                nn.GroupNorm(4, cnn_unit),
                nn.GELU(),
                nn.Dropout2d(0.25),

                nn.Conv2d(cnn_unit, cnn_unit*2, 3,  padding = 1, stride = (2,1)),
                nn.GroupNorm(4, cnn_unit*2),
                nn.GELU(),
                nn.Dropout2d(0.25),

                nn.Conv2d(cnn_unit*2, cnn_unit*4, 3,  padding = 1, stride = (2,1)),
                nn.GroupNorm(4, cnn_unit*4),
                nn.GELU(),
                nn.Dropout2d(0.25),
                nn.Conv2d(cnn_unit*4, cnn_unit*4, 3,  padding = 1),
                nn.GroupNorm(4, cnn_unit*4),
                )
        self.transformer = TransformerEncoder(cnn_unit*4, pos_embed_init_gamma=10.0, num_heads=8, fourier_size=16, hidden_factor=4, hidden_factor_attn=1, dropout=0.0, num_layers=4)
