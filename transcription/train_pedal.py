...
class Losses(nn.Module):
    def __init__(self):
        super().__init__()
        self.pedal_loss_fn = FocalLoss(alpha=1.0, gamma=2.0) # In: B x C x *

    def forward(self, logit, label):
        frame_loss = self.pedal_loss_fn(logit.permute(0, 2, 1), label)
...
def train_step(model, batch, loss_fn, optimizer, scheduler, device, config):
    ...
    loss = loss_fn(frame_out, shifted_pedal[:, 1:, 0])
...
def valid_step(model, batch, loss_fn, device, config):
    ...
    for n in range(audio.shape[0]):
        metrics = evaluate_pedal(frame_out[n].argmax(-1), shifted_pedal[n][1:,0])
        for k, v in metrics.items():
            validation_metric[k].append(v)
    validation_metric['pedal_loss'] = loss.mean(dim=(1))
...
def train(rank, world_size, config, ddp=True):
    ...
    model = PedalModel(config).to(rank)
                ...
                if step % config.valid_interval == 0 or step == 5000:
                    ...
                    with th.no_grad():
                        for n_valid, batch in enumerate(data_loader_valid):
                            batch_metric, _ = valid_step(model, batch, loss_fn, device, config)
                    ...
                    if rank == 0:
                        ...
                        model_saver.update(model, optimizer, step, valid_mean['metric/pedal/f1'], ddp=ddp)
                    if ddp:
                        dist.barrier()
            if step > config.iteration:
                break
