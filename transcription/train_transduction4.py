...
def train_step(model, pretrain_model, batch, loss_fn, optimizer, scheduler, device, config, cond_ratio, tf_ratio):
    ...
    frame_out = batch['pred_r'].to(device)

    # scaler.scale(total_loss).backward()
    # conditioned step
    if 0.0< tf_ratio < 1.0:
        selecter = (th.rand(label.shape[0], label.shape[1], 88) < tf_ratio).to(device)
        cond_frame = selecter * label + ~selecter * frame_out.detach()
    elif tf_ratio == 1.0:
        cond_frame = label
    elif tf_ratio == 0.0:
        cond_frame = frame_out.detach()
...
def test_step(model, pretrain_model, batch, device):
    ...
    for seg in tqdm(range(n_seg)):
        ...
        frame_init = batch['pred_r'].to(device)
        # sampling step
        cond_frame = frame_init.detach()
    ...
    return test_metric, out_iter_64 
...
def train(rank, world_size, config, ddp=True):
    ...
    optimizer = AdaBelief(model.parameters(), lr=config.lr, weight_decay=1e-4,
                          eps=1e-12, betas=(0.9,0.999), weight_decouple=False, 
    ...
    if not config.eval:
        ...
        for epoch in range(10000):
            ...
            for batch in data_loader_train:
                ...
                loss_cond = train_step(model, pretrain_model, batch, loss_fn, optimizer, scheduler, device, config, cond_ratio, tf_ratio)
                if rank == 0:
                    run.log({"train": dict(frame_loss_cond=loss_cond.mean())}, step=step)
                del batch, loss_cond
                    ...
                    if rank == 0:
                        ...
                        for key, value in valid_mean.items():
                            if key[-2:] == 'f1' or 'loss' in key or key[-3:] == 'err':
                                print(f'{key} : {value}')
                        valid_mean['metric/note/f1_iter16']
                        model_saver.update(model, optimizer, step, valid_mean['metric/note-with-offsets/f1_iter70'], ddp=ddp)
                    if ddp:
                        dist.barrier()
            if step > config.iteration:
                break
    ...
    with th.no_grad():
        for batch in iterator:
            batch_metric, preds = test_step(model, pretrain_model, batch, device)
            for k, v in batch_metric.items():
